import { money, read, uid, write } from "./store.js";
...
export function quoteDelivery(address, subtotal = 0) {
  ...
  const value = sanitize(address).toLowerCase();
  const postcode = value.match(/\b\d{5}\b/)?.[0] || "";
  if (value.includes("outside") || postcode.startsWith("99")) return { available: false, fee: 0, eta: "", area: "Outside range", message: "Sorry, we do not deliver to this address yet." };
  const zones = read("deliveryZones");
  let zone = zones[0];
  if (value.includes("midtown") || postcode.startsWith("10")) zone = zones[1] || zone;
  if (value.includes("outer") || value.includes("ring") || postcode.startsWith("11")) zone = zones[2] || zone;
  return {
    available: true,
    fee: subtotal >= settings.freeDeliveryThreshold ? 0 : Number(zone.fee || 0),
    eta: zone.eta || "30-40 min",
    area: zone.name,
    message: `${zone.name} delivery ${subtotal >= settings.freeDeliveryThreshold ? "is free" : `is ${money(zone.fee)}`}`,
  };
}
...
export function createOrderFromCart({ cart, context, customer, notes = "", paymentMethod = "Cash", voucher = null }) {
  ...
  const quote = context.type === "delivery" ? quoteDelivery(context.address, subtotal) : { available: true, fee: 0, area: "Pickup", eta: "" };
  ...
  return {
    ...
    address: context.type === "delivery" ? context.address : "",
